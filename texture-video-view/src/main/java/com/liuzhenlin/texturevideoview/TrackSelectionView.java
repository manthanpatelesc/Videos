...
public class TrackSelectionView extends LinearLayout {
  ...
  private final class TrackListAdapter extends RecyclerView.Adapter<TrackListAdapter.ViewHolder>
    ...
    void updateDataSet() {
      ...
      mTrackSelection =
          mVideoPlayer == null
              ? IVideoPlayer.INVALID_TRACK_INDEX : mVideoPlayer.getSelectedTrackIndex(mTrackType);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
      ...
      if (position == 0) {
        ...
        holder.checkedText.setText(getTrackNameProvider().getTrackName(mTrackGroup.valueAt(position)));
      }
    }
    ...
    public void onClick(View v) {
      ...
      if (trackSelection != oldTrackSelection) {
        ...
        if (mTrackSelection == trackSelection) {
          notifyItemChanged(mTrackGroup.indexOfKey(oldTrackSelection), PAYLOAD_CHECKED_STATE_CHANGED);
          ...
          if (mTrackSelection != oldTrackSelection) {
            notifyItemChanged(mTrackGroup.indexOfKey(oldTrackSelection), PAYLOAD_CHECKED_STATE_CHANGED);
            notifyItemChanged(mTrackGroup.indexOfKey(mTrackSelection), PAYLOAD_CHECKED_STATE_CHANGED);
          }
