 ...
 * <p>
 * This class requires the permission(s):
 * <ul>
 * <li>{@link android.Manifest.permission#READ_EXTERNAL_STORAGE} for a local audio/video file</li>
 * <li>{@link android.Manifest.permission#READ_EXTERNAL_STORAGE} to network based streaming content</li>
 * </ul>
 *
...
public abstract class AbsTextureVideoView extends DrawerLayout implements VideoPlayerControl {
    ...
    public interface OpCallback {
         ...
         * in the playlist.
         */
        default boolean canSkipToPrevious() {
            return false;
        }
    ...
    public interface OnLockedChangeListener {
        /**
         * Called when the lock state of this view changes
         *
         * @param locked indicates whether or not this view is locked
         */
        void onLockedChange(boolean locked);
    }
    ...
    /** If the controls are showing, this is marked. */
    ...
    /** Flag indicates whether this view is in locked mode or not. */
    ...
    /**
     * If set, the video will be forced to be stretched to fit the layout size in fullscreen, so it
     * may be distorted if its aspect ratio is unequal to the current view's.
     */
    ...
    private final OnClickListener mOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (id == R.id.bt_stretchVideo) {
                setVideoStretchedToFitFullscreenLayoutInternal(((Switch) v).isChecked(), false);
...
//                                }
    ...
    @Nullable
    private OnLockedChangeListener mOnLockedChangeListener;
    ...
    /* package-private */ final int mMaxVolume;
    ...
    public AbsTextureVideoView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressLint("ClickableViewAccessibility")
    public AbsTextureVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void checkSkipNextAndChooseEpisodeWidgetsVisibilities() {
        ...
        mChooseEpisodeButton.setVisibility((canSkipToPrevious || canSKipToNext) ? VISIBLE : GONE);
    }
    ...
    public void setSingleVideoLoopPlayback(boolean looping) {
        ...
        if (mMoreView != null) {
            Switch toggle = mMoreView.findViewById(R.id.bt_loopSingleVideo);
    ...
    /**
     * Sets the listener to get notified when the lock mode of this view changes.
     *
    ...
    private void setVideoStretchedToFitFullscreenLayoutInternal(boolean stretched, boolean checkSwitch) {
        ...
        if (checkSwitch && mMoreView != null) {
            Switch toggle = mMoreView.findViewById(R.id.bt_stretchVideo);
            if (stretched != toggle.isChecked()) {
                toggle.setChecked(stretched);
            }
        }
        if (isInFullscreenMode() && !isClipViewBounds() && mVideoWidth != mVideoHeight) {
    ...
    public final void setFullscreenMode(boolean fullscreen, int navTopInset) {
        try {
            ...
            if (fullscreen) {
                ...
                } else if (mMoreView == null && isDrawerVisible(mDrawerView)) {
    ...
    protected void onDetachedFromWindow() {
        ...
        mHideBrightnessOrVolumeFrameRunnable.run();
        if (mTimedOffRunnable != null) {
            removeCallbacks(mTimedOffRunnable);
            mTimedOffRunnable = null;
        }
        ...
        if (locked != isLocked()) {
            ...
            if (fullscreen && showing) {
                if (animate && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    TransitionManager.beginDelayedTransition(mTopControlsFrame, new Fade());
                    TransitionManager.beginDelayedTransition(mBottomControlsFrame,
                            new TransitionSet()
            ...
            if (mOnLockedChangeListener != null) {
                mOnLockedChangeListener.onLockedChange(locked);
            }
        }
    }
