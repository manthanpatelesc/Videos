...
public class VideoActivity extends SwipeBackActivity {
        ...
        mVideoView.setVideoListener(new AbsTextureVideoView.VideoListener() {
            ...
            public void onVideoStopped() {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && isInPictureInPictureMode()) {
                    updatePictureInPictureActions(PIP_ACTION_FAST_REWIND
                            | PIP_ACTION_PLAY | PIP_ACTION_FAST_FORWARD);
                }
            }
    ...
    public void onAttachedToWindow() {
        ...
        // 初始化布局
        if (ViewCompat.getLayoutDirection(decorView) == ViewCompat.LAYOUT_DIRECTION_RTL) {
    ...
    public void onBackPressed() {
        if (mVideoView.isDrawerOpen(GravityCompat.END)) {
            mVideoView.closeDrawer(GravityCompat.END);
            // 如果是全屏状态，退出全屏状态而不是关闭此页面
        } else if (mVideoView.isInFullscreenMode()) {
    ...
    private void setScreenOrientationLocked(boolean locked) {
        if (locked) {
            ...
            mLockUnlockOrientationButton.setContentDescription(sUnlockOrientation);
        } else {
            mPrivateFlags &= ~PFLAG_SCREEN_ORIENTATION_LOCKED;
            mLockUnlockOrientationButton.setImageResource(R.drawable.ic_lock);
            mLockUnlockOrientationButton.setContentDescription(sLockOrientation);
            // 取消锁定则将屏幕方向设为当前设备方向
    ...
    public void onPictureInPictureModeChanged(boolean isInPictureInPictureMode) {
        ...
        if (isInPictureInPictureMode) {
            updatePictureInPictureActions(PIP_ACTION_FAST_REWIND
                    | (mVideoView.isPlaying() ? PIP_ACTION_PAUSE : PIP_ACTION_PLAY)
    ...
    private class VideoEpisodesAdapter extends AbsTextureVideoView.PlayListAdapter<VideoEpisodesAdapter.ViewHolder> {
                ...
                for (Object payload : payloads) {
                    if (payload == sRefreshVideoProgressPayload) {
                        ...
                        if (position == mVideoIndex) {
                            holder.videoProgressDurationText.setText(sWatching);
                        } else {
                            holder.videoProgressDurationText.setText(
                                    VideoUtils.jointVideoProgressAndDuration(
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            if (position == mVideoIndex) {
                ...
                holder.videoProgressDurationText.setText(
                        VideoUtils.jointVideoProgressAndDuration(video.getProgress(), video.getDuration()));
            }
        }
