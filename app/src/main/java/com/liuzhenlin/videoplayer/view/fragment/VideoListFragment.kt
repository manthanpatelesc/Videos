...
//        notifyListenersOnReloadVideos()
        ...
        when (requestCode) {
            ...
            REQUEST_CODE_FOLDED_VIDEOS_FRAGMENT ->
                if (resultCode == RESULT_CODE_FOLDED_VIDEOS_FRAGMENT) {
                    val directory = data?.getStringExtra(KEY_DIRECTORY_PATH) ?: return
                    val videos = data.getParcelableArrayListExtra<Video>(KEY_VIDEOS)
                    for ((i, item) in mVideoListItems.withIndex()) {
                        if (item.path != directory) continue
                        ...
                        when (videos.size) {
                            ...
                            1 -> {
                                ...
                                if (item is Video) {
                                    val vd = VideoDaoHelper.getInstance(mContext).queryVideoDirByPathOrInsert(directory)
                                    vd.videos = videos
    ...
    private inner class VideoListAdapter : RecyclerView.Adapter<VideoListAdapter.VideoListViewHolder>() {
            ...
            if (payloads.isEmpty()) {
                ...
                if (payload and PAYLOAD_REFRESH_VIDEO_PROGRESS_DURATION != 0) {
                    ...
                    (holder as VideoViewHolder).videoProgressAndDurationText.text =
                            VideoUtils.jointVideoProgressAndDuration(progress, duration)
                }
            ...
            when (getItemViewType(position)) {
                VIEW_TYPE_VIDEO -> {
                    ...
                    vh.videoProgressAndDurationText.text =
                            VideoUtils.jointVideoProgressAndDuration(video.progress, video.duration)
                }
                VIEW_TYPE_VIDEODIR -> {
                    val vh = holder as VideoDirViewHolder
                    val videos = (item as VideoDirectory).videos
        ...
        private fun separateToppedItemsFromUntoppedOnes(holder: VideoListViewHolder, position: Int) {
            ...
            if (mVideoListItems[position].isTopped) {
                ...
                holder.topButton.text = STRING_CANCEL_TOP
            } else {
                ViewCompat.setBackground(holder.itemVisibleFrame,
                ...
                holder.topButton.layoutParams = lp
                holder.topButton.text = STRING_TOP
            }
        }
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.bt_selectAll -> {
                // 全选
                if (STRING_SELECT_ALL == mSelectAllButton!!.text.toString()) {
                    for (i in mVideoListItems.indices) {
    ...
    private fun onItemCheckedChange() {
        ...
        if (checkedItemCount == mVideoListItems.size) {
            mSelectAllButton!!.text = STRING_SELECT_NONE
        } else {
            mSelectAllButton!!.text = STRING_SELECT_ALL
        }
    ...
    private fun showItemDetailsDialog(item: VideoListItem) {
        ...
        if (item is Video) {
            ...
            videoNameText.text = getString(R.string.name, item.name)

            val videoSizeText = view.findViewById<TextView>(R.id.text_videoSize)
            videoSizeText.text = getString(R.string.size, FileUtils.formatFileSize(item.size.toDouble()))

            val videoScaleText = view.findViewById<TextView>(R.id.text_videoScale)
            videoScaleText.text = getString(R.string.scale, VideoUtils.formatVideoScale(item.width, item.height))

            val videoPathText = view.findViewById<TextView>(R.id.text_videoPath)
            videoPathText.text = getString(R.string.path, item.path)
        } else /* if (item is VideoDirectory) */ {
            ...
            videodirNameText.text = getString(
                    if (item.name.equals(dirname, ignoreCase = true)) R.string.name else R.string.alias, item.name)

            val videodirSizeText = view.findViewById<TextView>(R.id.text_videodirSize)
            videodirSizeText.text = getString(R.string.size, FileUtils.formatFileSize(item.size.toDouble()))

            val videoCountText = view.findViewById<TextView>(R.id.text_videoCount)
            videoCountText.text = getString(R.string.videoCount, videos.size)

            val videodirPathText = view.findViewById<TextView>(R.id.text_videodirPath)
            videodirPathText.text = getString(R.string.path, path)
        }
    ...
    companion object {
        internal const val PAYLOAD_CHANGE_ITEM_LPS_AND_BG = 1
        internal const val PAYLOAD_CHANGE_CHECKBOX_VISIBILITY = 1 shl 1
        internal const val PAYLOAD_REFRESH_CHECKBOX = 1 shl 2
        internal const val PAYLOAD_REFRESH_CHECKBOX_WITH_ANIMATOR = 1 shl 3
        internal const val PAYLOAD_REFRESH_ITEM_NAME = 1 shl 4
        internal const val PAYLOAD_REFRESH_VIDEO_PROGRESS_DURATION = 1 shl 5

        private const val PAYLOAD_REFRESH_VIDEODIR_THUMB = 1 shl 6
        private const val PAYLOAD_REFRESH_VIDEODIR_SIZE_AND_VIDEO_COUNT = 1 shl 7
    }
}